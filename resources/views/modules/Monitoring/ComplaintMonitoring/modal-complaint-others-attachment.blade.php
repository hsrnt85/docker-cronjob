        ...
        @foreach($complaintAttachment as $attachment)
            @if ( $attachment)
                 <input type="hidden" value="{{$attachment->id}}" name="attachment_id" id="attachment_id" >
                    <img src="{{ getCdn().'/'.$attachment->path_document }}" class="mb-1" width= "50px" height="50px"  onclick="showOtherComplaintImage(this,{{ $attachment->id}});" style="border: 1px solid black" >
            @endif
        @endforeach
    @endif
</div>
