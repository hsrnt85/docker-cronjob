                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <table id="datatable" class="table table-striped table-bordered dt-responsive wrap w-100 dataTable" role="grid">
                                ...
                                <tbody>
                                    @foreach($senaraiJabatan as $bil => $data)
                                        <tr>
                                            <th class="text-center" scope="row">{{ ++$bil }}</th>
                                            <td>{{ upperText($data -> agency -> name) }}</td>
                                            <td>{{ upperText($data -> department_name) }}</td>
                                            <td class="text-center">
                                                <div class="btn-group" role="group">
                                                    @if(checkPolicy("V"))
                                                    <a href="{{ route('department.view', ['id' => $data->id]) }}" class="btn btn-outline-primary px-2 py-1 tooltip-icon">
                                                        <span class="tooltip-text">{{ __('button.papar') }}</span> <i class="{{ __('icon.view_folder') }}"></i>
                                                    </a>
                                                    @endif
                                                    {{-- <a href="" class="btn btn-outline-primary px-2 py-1 tooltip-icon">
