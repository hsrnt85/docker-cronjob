                                ...
                                <tbody>
                                    @foreach ($quartersCategoryAll as $quartersCategory)
                                        <tr class="odd">
                                            ...
                                            <td class="text-center">{{ $quartersCategory->name }} </td>
                                            <td class="text-center">{{ $quartersCategory->district->district_name }} </td>
                                            <td class="text-center">{{ $quartersCategory->landed_type->type }} </td>
                                            <td class="text-center">
                                                <div class="btn-group" role="group">
                                                    @if(checkPolicy("V"))
                                                        <a href="{{ route('routineInspectionRecord.listInspection', $quartersCategory) }}" class="btn btn-outline-primary px-2 py-1 tooltip-icon">
