...
<div class="row">
    <div class="col-12">
        ...
        <div class="card">
            <div class="card-body">
                {{-- PAGE TITLE --}}
                <div class="border-bottom border-primary mb-4"><h4 class="card-title">Carta Aduan Awam</h4></div>
                            ...
                            <table id="my-table" class="table table-striped table-bordered dt-responsive wrap w-100 dataTable" role="grid">
                                <thead class="bg-primary bg-gradient text-white">
                                    <tr role="row">
                                        ...
                                        <th class="text-center" >Dalam Pengesahan</th>
                                        <th class="text-center" >Dalam Tindakan</th>
                                        <th class="text-center" >Dalam Pemantauan <br> Berulang</th>
                                        <th class="text-center" >Aduan <br/> Selesai</th>
                                        <th class="text-center" >Aduan <br/> Ditolak</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($complaintStatisticByQuartersCategory as $category)

                                        <tr class="text-center">
                                            <td>{{ $loop->iteration }}</td>
                                            <td>{{ $category->name }}</td>
                                            <td>{{ $category->quarters->sum('total_complaints') }}</td>
                                            <td>{{ $category->quarters->sum('new_complaints') }}</td>
                                            <td>{{ $category->quarters->sum('active_complaints') }}</td>
                                            <td>{{ $category->quarters->sum('recurring_complaints') }}</td>
                                            <td>{{ $category->quarters->sum('done_complaints') }}</td>
                                            <td>{{ $category->quarters->sum('rejected_complaints') }}</td>
                                        </tr>
                                    ...
                                    <tr class="bg-secondary bg-gradient text-white">
                                        ...
                                        <td class="text-center"><strong>{{ $gtComplaints }}</strong></td>
                                        <td class="text-center"><strong>{{ $gtNewComplaints }}</strong></td>
                                        <td class="text-center"><strong>{{ $gtActiveComplaints }}</strong></td>
                                        <td style="text-align: center"><strong>{{ $gtRecurringComplaints }}</strong></td>
                                        <td class="text-center"><strong>{{ $gtDoneComplaints }}</strong></td>
                                        <td class="text-center"><strong>{{ $gtRejectedComplaints }}</strong></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
...
@section('script')
<script src="{{ URL::asset('assets/js/pages/Analysis/RulesViolationComplaintAnalysis/rulesViolationComplaintAnalysis.js')}}"></script>
@endsection
