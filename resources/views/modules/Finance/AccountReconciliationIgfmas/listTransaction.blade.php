...
<div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <form  class="custom-validation" id="form" method="post" action="{{ route('accountReconciliationIgfmas.processPayment', ['year' => $year, 'month' => $month]) }}" >
                {{ csrf_field() }}
                {{-- PAGE TITLE --}}
                ...
                <div class="row mb-2">
                    <div class="col-sm-12">
                        @if(checkPolicy("U"))
                            @if($payment_status == 1)
                                <a href="{{route('accountReconciliationIgfmas.getPaymentReceipt', ['year' => $year, 'month' => $month]) }}" target="_blank" class="btn btn-success float-end waves-effect waves-light me-2" >
                                    {{ __('button.cetak_resit') }}
                                </a>
                            @endif
                        ...
                        @endif
                        @if(checkPolicy("A"))
                            <a href="{{ route('accountReconciliationIgfmas.create', ['year' => $year, 'month' => $month]) }}" class="btn btn-success float-end waves-effect waves-light me-2">{{ __('button.rekod_baru') }}</a>
                        @endif
                ...
                <div id="datatable_wrapper">
                    <div class="row">
                        <div class="col-sm-12">
                                        ...
                                        <tr class="odd">
                                            ...
                                            <td class="text-center">
                                                <div class="btn-group" role="group">
                                                    @if(checkPolicy("V"))
                                                        <a href="{{ route('accountReconciliationIgfmas.view', ['id' => $data->id, 'year' => $year, 'month' => $month]) }}" class="btn btn-outline-primary px-2 py-1 tooltip-icon">
                            ...
                            <div class="mt-3 row">
                                <div class="col-sm-12">
                                    <a href="{{ route('accountReconciliationIgfmas.listYearMonth', ['year' => $year, 'month' => $month]) }}" class="btn btn-secondary float-end me-2">{{ __('button.kembali') }}</a>
                                </div>
                            </div>
            ...
            <form method="POST" action="{{ route('accountReconciliationIgfmas.deleteByRow') }}" id="delete-form-by-row">
                {{ csrf_field() }}
                {{ method_field('DELETE') }}
...
@section('script')
<script src="{{ URL::asset('assets/js/pages/AccountReconciliationIgfmas/accountReconciliationIgfmas.js')}}"></script>
@endsection
