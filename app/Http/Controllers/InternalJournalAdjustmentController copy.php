...
use App\Models\InternalJournalVotList;
...
class InternalJournalAdjustmentControllerx extends Controller
{
    ...
    public function create()
    {
        ...
        $tenants_payment_notice = TenantsPaymentNotice::get_payment_notice_by_district(districtId());//dd($tenants_payment_notice);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach($request->income_code as $i => $income_code){
                $internalJournalVotList = new InternalJournalVotList;
                $internalJournalVotList->internal_journal_id       = $internalJournal->id;
                $internalJournalVotList->income_account_code_id    = $income_code;
                $internalJournalVotList->debit_amount              = (isset($request->debit[$i])) ? $request->debit[$i] : 0;
                $internalJournalVotList->credit_amount             = (isset($request->credit[$i])) ? $request->credit[$i] : 0;
                $internalJournalVotList->data_status               = 1;
                $internalJournalVotList->action_by                 = loginId();
                $internalJournalVotList->action_on                 = currentDate();
                $internalJournalVotList->save();
            }
    ...
    public function edit(Request $request)
    {
        ...
        $internal_journal_vot_list  = InternalJournalVotList::get_internal_journal_vot_list($id);
        ...
        $tenants_payment_notice = TenantsPaymentNotice::get_payment_notice_by_district(districtId());
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            if($request->proses_sedia == 1 || $current_status == 1)
            {
                ...
                $get_vot_list    = InternalJournalVotList::where(['internal_journal_id' => $id, 'data_status' => 1])->get();
                ...
                foreach ($income_code_arr as $i => $income_code_id) {
                    if ($i < $total_vot_list) { //update
                        ...
                        $vot_list->internal_journal_id = $internal_journal->id;
                        $vot_list->income_account_code_id = $income_code_id;
                        $vot_list->debit_amount = isset($request->debit[$i]) ? $request->debit[$i] : 0;
                        $vot_list->credit_amount = isset($request->credit[$i]) ? $request->credit[$i] : 0;
                        $vot_list->data_status = 1;
                        $vot_list->action_by = loginId();
                        $vot_list->action_on = currentDate();
                        $vot_list->save();
                    } else {
                        $insert = new InternalJournalVotList;
                        $insert->internal_journal_id = $internal_journal->id;
                        $insert->income_account_code_id = $income_code_id;
                        $insert->debit_amount = isset($request->debit[$i]) ? $request->debit[$i] : 0;
                        $insert->credit_amount = isset($request->credit[$i]) ? $request->credit[$i] : 0;
                        $insert->data_status = 1;
                        $insert->action_by = loginId();
                        $insert->action_on = currentDate();
                        $insert->save();
                    }
                }
            ...
            elseif($request->proses_lulus == 1)
            {
                ...
                TenantsPaymentNoticeVotList ::where('tenants_payment_notice_id' , $internal_journal->tenants_payment_notice_id)
                ->update(['internal_journal_id' => $id, 'adjustment_amount' => $internal_journal->adjustment_amount, 'total_amount_after_adjustment' => $internal_journal->final_amount_hidden]);
    ...
    public function destroyByRow(Request $request)
    {
        ...
        try {
            ...
            InternalJournalVotList::where('id', $vot_list_id)
                                    ->update([
                                        'data_status' => 0,
                                        'delete_by' => loginId(),
    ...
    private function _generateRefNo($running_no, $district)
    {
        ...
        $ref_no = currentYearTwoDigit(). $district->finance_district_code . 'JPD05' . $ref_no;
    ...
    private function _saveTenantsPaymentNoticeVotList($tenants_payment_notice_id, $vot_id, $amount)
    {   //dd($dataIncomeAccountCode);
        ...
        $dataIncomeAccountCode = IncomeAccountCode::from('income_account_code as iac')
            ...
            ->where(['iac.data_status'=> 1,'pc.data_status'=> 1, 'iac.id'=>$vot_id])
