...
class ReconciliationTransactionIspeks extends Model
{
    use HasFactory;
    protected $table = 'reconciliation_transaction_ispeks';
    ...
    {
        $data = self::from('reconciliation_transaction_ispeks as rt')->join('month','rt.month','=','month.id')->select('month.name','month.id')->groupBy('month')->get();
        return $data;
    }

    public static function salary_deduction_sode_summary($year, $month)
    {
        $data = self::from('reconciliation_transaction_ispeks as rt')
    ...
    {
        $data = self::from('reconciliation_transaction_ispeks as rt')
            //->leftJoin("tenants_payment_transaction as tpt", DB::raw('FIND_IN_SET(rt.id, tpt.reconciliation_transaction_id)'),">", DB::raw("'0'"))
    ...
    public static function reconciliation_transaction_item($salary_deduction_code, $year, $month)//$id
    {
        $data = self::from('reconciliation_transaction_ispeks as rt')
            ...
            ->where('rt.data_status', 2)->where('rti.data_status', 2)//->where('rti.reconciliation_transaction_ispeks_id', $id)
            ->orderBy('ic_no','ASC')
    ...
    public static function intersect_reconciliation_transaction_item($salary_deduction_code, $year, $month)//$id
    {
        $data = self::from('reconciliation_transaction_ispeks as rt')
            ->join('reconciliation_transaction_ispeks_item as rti','rt.id','=','rti.reconciliation_transaction_ispeks_id')
            ->selectRaw('rti.ic_no, sum(rti.amount) AS amount')//
            ->where('salary_deduction_code', $salary_deduction_code)->where('year', $year)->where('month', $month)
            ->where('rt.data_status', 2)->where('rti.data_status', 2)//->where('rti.reconciliation_transaction_ispeks_id', $id)
            ->orderBy('ic_no','ASC')
            ->orderBy('salary_deduction_code','ASC')
